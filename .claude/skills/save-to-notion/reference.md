...
**分離 vs 連結の判断基準**:
- 意味的に別々の項目（案A・案B、複数の異なる発言など）→ それぞれ別 `>` ブロックに分ける
- 1つの概念・発言が複数行にわたる場合 → `<br>` で連結して1ブロックにまとめる
...
**重要**: 子要素をインデントしないとトグル内に含まれない。インデントはタブ1つ（`\t`）で統一する。

**形式の選択**: 元コンテンツに `<details>/<summary>` がある場合はそのまま使用する。新規に Toggle を作成する場合は `<details>` 形式を優先する。
...
| 同一概念の複数行引用を `>` 行で分離 | `<br>` で結合して1つの `>` にまとめる（意味的に別項目は別ブロックのまま。§2「分離 vs 連結の判断基準」を参照） |
