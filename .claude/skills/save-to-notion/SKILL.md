...
**「明示」の判定基準**:
- 「さっきの調査結果を」「この分析を」「上の比較を」→ 範囲が特定できる → 明示扱い（確認不要）
- 直近の会話が複数の無関係なトピックにわたっており、どれを指しているか不明 → 確認必要

**明示の場合のタイトル**:
範囲が明示扱いの場合、ページタイトルは内容から自動生成して進める（ユーザーへの確認不要）。
...
主要な変換ルールは `~/.claude/skills/save-to-notion/reference.md` も参照。
...
主な変換ポイント（詳細は `~/.claude/skills/save-to-notion/reference.md` 参照）:
...
**callout の代替方法**（優先順位順）:
1. `**太字**` + 見出し（`##`, `###`）— 最初にこれを試みる
2. `<span color="orange">テキスト</span>` 等のカラー指定 — 色による視覚的強調が必要な場合のみ使用
...
```json
{
  "parent": {
    "type": "data_source_id",
    "data_source_id": "31237b63-98eb-8057-abd4-000b8a76b8cb"
  },
  "pages": [
    {
...
Notion-flavored Markdownの変換ルール詳細は `~/.claude/skills/save-to-notion/reference.md` を参照。
