...
- **必ず`/Users/shifumi.fukunaga/ghq/github.com/kufu/monjo/`ディレクトリ以下を調査する**
- monjoリポジトリにはSmartHRの操作方法、機能説明、設定手順などのドキュメントが含まれている
- 文書の検索時は以下のツールを使用：
  - `Grep`ツール：キーワード検索（path: `/Users/shifumi.fukunaga/ghq/github.com/kufu/monjo`を指定）
  - `Glob`ツール：ファイル名パターン検索（path: `/Users/shifumi.fukunaga/ghq/github.com/kufu/monjo`を指定）
