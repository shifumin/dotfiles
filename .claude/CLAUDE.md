...
### 必須の文字エンコーディング設定
- **すべてのファイルはUTF-8で保存する** - 文字化けを防ぐため、必ずUTF-8エンコーディングを使用
- **BOMなしUTF-8を使用** - BOM（Byte Order Mark）は付けない
- **改行コードはLFを使用** - Unix/Linux/Mac標準のLF（\n）を使用

### ファイル作成・編集時の注意事項
1. **新規ファイル作成時** - WriteツールやEditツールを使用する際は、自動的にUTF-8で処理される
2. **既存ファイル編集時** - 文字化けが検出された場合は、即座に作業を中断してユーザーに報告
3. **日本語を含むファイル** - 特に注意して、UTF-8エンコーディングが維持されていることを確認

### 文字化けが発生した場合の対処
1. **文字化けを検出したら即座に停止** - それ以上の編集を行わない
2. **ユーザーに報告** - 文字化けが発生していることを明確に伝える
3. **UTF-8で再書き込み** - ユーザーの許可を得てから、正しいエンコーディングで書き直す

### プロジェクト設定の確認
```bash
# .editorconfigファイルがある場合は確認
cat .editorconfig

# gitの設定を確認
git config core.quotepath false  # 日本語ファイル名の文字化け防止
```
