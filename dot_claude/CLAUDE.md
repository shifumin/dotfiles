...
| 観点 | 説明 | 具体例 |
|------|------|--------|
| **簡潔性** | 不要・冗長・重複を削る | 自明な内容は省略、同じ説明を複数箇所に書かない |
| **明示性** | 条件・理由・例外を明確に | 指示には「なぜ」を1文追加、条件分岐を明示 |
| **構造化** | 見出し・箇条書き・テーブルで整理 | `#`→`##`→`###`の順、`-`統一、比較はテーブル、30行超は分割 |
| **十分性** | 目的達成に必要な情報が揃っているか | 不足している前提・手順・例外がないか確認 |
| **一貫性** | 用語・表現を統一 | 同じ概念に異なる名称を使わない |
...
| ルール | 説明 |
|--------|------|
| **パスにユーザー名をハードコードしない** | `/Users/shifumin/`ではなく`~/`を使用（端末依存を避けるため） |
