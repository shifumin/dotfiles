...
| ユーザーの意図 | 操作 |
|--------------|------|
| コードの分析・質問・生成 | `codex exec` |
| コード変更のレビュー | `codex review` |
...
3. ユーザーの意図に応じてプロンプト末尾にサフィックスを付加（下表参照）
4. `codex exec`をBashツールで実行（timeout: 600000）
5. 出力結果をユーザーに報告

### プロンプトサフィックス

Codexが適切な粒度で回答するよう、ユーザーの意図に応じてプロンプト末尾に指示を付加する。

| ユーザーの意図 | サフィックス |
|--------------|------------|
| 分析・改善提案（「〜を分析して」「改善案ある？」） | 「具体的な提案・修正案・コード例まで自主的に出力してください。」 |
| 事実確認・状態把握（「〜はどうなってる？」「〜を確認して」） | 付加しない |
| 比較・選択（「AとBどっちがいい？」） | 「それぞれのメリット・デメリットを比較してください。」 |

迷ったら付加しない。Codexは十分な応用力を持つため、過剰な指示より簡潔なプロンプトの方がよい結果を出しやすい。

### 実行コマンド

短いプロンプトの場合:
```bash
/opt/homebrew/bin/codex exec --full-auto --sandbox read-only --cd {ディレクトリパス} "{プロンプト}"
```
...
Codexの実行結果を受け取った後、以下の基準で対応を判断する。

| ユーザーの意図 | 対応 |
|--------------|------|
| 分析・質問（「〜を調べて」「〜はどうなってる？」） | Codexの出力を要約し、重要なポイントを整理して報告 |
| レビュー（「レビューして」「問題点を指摘して」） | 指摘事項をカテゴリ別に整理して報告。修正は提案のみで、適用はユーザーの指示を待つ |
| コード生成・修正提案（「〜を書いて」「〜を直して」） | Codexの提案内容を示し、適用するかユーザーに確認してから実行 |

### 原則

- Codexの出力をそのまま丸投げしない。Claude Code側で整理・要約する
- Codexが提案したコード変更を自動適用しない。ユーザーの承認を得てからEditツール等で適用する
- Codexの出力が長大な場合は、要点を抽出して報告し、詳細が必要か確認する
